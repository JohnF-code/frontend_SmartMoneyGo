...
const ListItem = ({ client, prestamos }) => {
    ...
    // Función para determinar el estado (semaforo)
    const semaforo = (diasAtraso) => {
        if (diasAtraso > 10) {
            return (
                <span className="rounded-3xl py-2 px-3 text-sm font-bold text-white bg-red-600">
                    {diasAtraso} días de atraso
                </span>
            );
        } else if (diasAtraso > 0) {
            return (
                <span className="rounded-3xl py-2 px-3 text-sm font-bold text-white bg-yellow-400">
                    {diasAtraso} días de atraso
                </span>
            );
        } else {
            return (
                <span className="rounded-3xl py-2 px-3 text-sm font-bold text-white bg-teal-400">
                    Puntual
                </span>
            );
        }
    ...
    useEffect(() => {
        getPayments(); // Carga inicial de pagos
    }, []);

    useEffect(() => {
        calcularEstadoGlobal(); // Actualizar el estado de atraso
    }, [payments]);
    ...
    const calcularEstadoGlobal = () => {
        ...
        prestamos.forEach((prestamo) => {
            const currentPayments = payments.filter(
                (payment) => payment.loanId?._id === prestamo._id
            );

            if (currentPayments.length > 0) {
                const mostRecentPayment = getMostRecentPayment(currentPayments);
                const dias = calcularDiasAtraso(prestamo.finishDate, mostRecentPayment?.date);
    ...
    return (
        <li>
            <ul className="rounded-lg flex items-center justify-around accordion border-b border-grey-light hover:bg-gray-100 dark:hover:bg-blue-800" onClick={() => setPanel(!panel)}>
                ...
                <li className="inline-flex items-center">
                    <span className="py-3 w-40">
                        <p className="text-gray-800 text-sm dark:text-gray-400">{name}</p>
                        <p className="hidden md:table-cell text-xs text-gray-500 font-medium dark:text-white">Nombre</p>
                    </span>
                </li>
                <li className="hidden md:table-cell">
                    <p className="text-sm text-gray-800 font-medium dark:text-gray-400">{document}</p>
                    <p className="text-xs text-gray-500 font-medium dark:text-white">Identidad</p>
                </li>
                <li className="hidden md:table-cell">
                    <p className="text-sm text-gray-800 font-medium dark:text-gray-400">{contact}</p>
                    <p className="text-xs text-gray-500 font-medium dark:text-white">Número telefonico</p>
                </li>
                <li className="hidden md:table-cell">
                    <p className="text-sm text-gray-700 font-medium dark:text-gray-400">{calcularTotalPrestado()}</p>
                    <p className="text-xs text-gray-500 font-medium dark:text-white">Dinero Prestado</p>
                </li>
                <li>
                    {!atraso ?
